...
def clean_output_dir(dir_name, noise_level, llm_iter=0, dir_path="debug_llm_outputs"):
    dir_path = os.path.join(PARENT_PATH, "pipeline", dir_path, dir_name, f"noise_level_{noise_level}", f"llm_iter_{llm_iter}")
...
def create_llm_iter_n_folders(dir_name, noise_level, llm_iter=0, dir_paths=None):
    if dir_paths is None:
        dir_paths = ["prompts", "debug_llm_outputs"]

    for path in dir_paths:
        path = os.path.join(PARENT_PATH, "pipeline", path, dir_name, f"noise_level_{noise_level}", f"llm_iter_{llm_iter}")
        os.makedirs(path, exist_ok=True)


def reset_prompt_to_init(dir_name, noise_level, llm_iter=0, dir_path="prompts"):
    source = os.path.join(PARENT_PATH, "pipeline", dir_path, "reset-for-continue.txt")
    target = os.path.join(PARENT_PATH, "pipeline", dir_path, dir_name, f"noise_level_{noise_level}", f"llm_iter_{llm_iter}", "continue-iter.txt")
    shutil.copyfile(source, target)

    refine_source = os.path.join(PARENT_PATH, "pipeline", dir_path, "reset-for-continue-refinement.txt")
    refine_target = os.path.join(PARENT_PATH, "pipeline", dir_path, dir_name, f"noise_level_{noise_level}", f"llm_iter_{llm_iter}", "continue-iter-refinement.txt")
